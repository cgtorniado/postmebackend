...
app.post('/postfeed', async (req,res)=> {
    const {userid} = req.body
    db.query("select posts.postid, posts.userid,  posts.content, posts.date_created, posts.date_updated, register.firstName, register.lastName, register.username from posts inner join register on posts.userid = register.userid where posts.userid=? order by posts.postid desc",[userid] , 
    (err,result) => {   
        if(err){
            console.log(err.message);
        }
