...
app.post('/newpostothernotif',  (req,res)=> {
    ...
    db.query("insert into notifications (notiftype,othertypeid,notifreceiverid,notifsenderid,new_comment) values (?,?,?,?,?)",[notiftype,postid,wallid,userid,"1"] , 
    (err,result) => {   
        if(err){
            console.log(err.message);
        }
...
app.post('/notiffeed',  (req,res)=> {
    const {userid} = req.body
    db.query(`select notif.notifid, notif.notiftype,notif.othertypeid,notif.notifreceiverid,notif.notifsenderid, notif.new_comment, notif.date_created,
    ...
    where notif.notifreceiverid = ? order by notif.date_created desc`,[userid] , 
    (err,result) => {   
        if(err){
            console.log(err.message);
        }
